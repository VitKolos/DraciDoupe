...
error_reporting(0);
	...
	if (!isset($_GET["upravit"]) && !isset($_GET["find"])) {
	$dotaz2="";
	$vystup2="";
	...
	for ($i = 0; $vystup2 = $dotaz2->fetch(); $i++) {
		...
		else if ($vystup2["pohlavi"]=="zena") {$barva = "red";}
		else {$barva = "grey";}
	...
	else if (isset($_GET["upravit"]) && !isset($_GET["find"])) {
	$dotaz2="";
	$vystup2="";
	$dotaz2 = $db->prepare("SELECT * FROM room".$_GET["id"]);
	$dotaz2->execute($parametry);
	...
	else if (!isset($_GET["upravit"]) && isset($_GET["find"]) && isset($_GET["hrac"])) {
	$dotaz2="";
	$vystup2="";
	$dotaz2 = $db->prepare("SELECT * FROM room".$_GET["id"]);
	$dotaz2->execute($parametry);
	echo "<small>Vyber s jakým hráčem bude bojovat:</small><br>";
	for ($i = 0; $vystup2 = $dotaz2->fetch(); $i++) {
		if ($vystup2["id"] != $_GET["hrac"]) {
		$barva = "black";
		if ($vystup2["pohlavi"]=="muz") {$barva = "blue";}
		else if ($vystup2["pohlavi"]=="zena") {$barva = "red";}
		else {$barva = "grey";}
		echo '<div onclick="bojovat('.$_GET["hrac"].', '.$vystup2["id"].');" style="color:white; display:inline-block; background-color:'.$barva.'";>'.htmlspecialchars($vystup2["jmeno"]).'</div>';
		echo "<br>";
		$nejacihraci=true;
		}
	}
