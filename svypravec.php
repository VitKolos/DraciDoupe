...
if (!isset($_GET["klic"]) || !isset($_SESSION["prihlaseni"]) || !isset($_SESSION["vypravec"])) {
...
if (!isset($nazev) || $_SESSION["owner"] != "s".$id) {
	header("Location: index.php");
	exit;
}
...
<html>
    <head> 
		...
		<?php if (isset($_GET["klic"])) {echo "<title>".htmlspecialchars($nazev)."</title>";} ?>
        ...
        <div style="text-align:center"><br><div style="height:75vh">
			<?php
				if (isset($_GET["klic"])) {
					...
					if (isset($_SESSION["owner"]) && $_SESSION["owner"] == "s".$id) {
						//u ownera
...
function editvar(typ, id, hodnota) {
  ...
  xhttp.open("GET", "rodeslat.php?klic='.$klic.'&hrac="+id+"&typ="+typ+"&hodnota="+hodnota+"&edit", true);
  xhttp.send();
	}
}
...
function upravit() {
	...
	xhttp.open("GET", "rhraci.php?klic='.$klic.'&upravit", true);
	xhttp.send();
...
function submit() {
  ...
  xhttp.send("klic='.$klic.'&autor=0&cas="+time+"&zprava="+message);
	  }
	  else {
		document.getElementById("timetick").style.display = "none";
		document.getElementById("tick").style.display = "none";
  ...
  xhttp.open("POST", "rodeslat.php", true);
  xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  xhttp.send("klic='.$klic.'&autor=0&cas="+time);
	  }
  }
  else {
...
function info() {
  ...
  xhttp.open("GET", "rinfo.php?klic='.$klic.'&owner", true);
  xhttp.send();
}
...
function hraci() {
  ...
  xhttp.open("GET", "rhraci.php?klic='.$klic.'", true);
  xhttp.send();
}
...
function chat() {
  ...
  xhttp.open("GET", "rchat.php?klic='.$klic.'&hrac=0", true);
  xhttp.send();
}
