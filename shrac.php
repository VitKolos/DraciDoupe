...
if (!isset($_GET["klic"]) || !isset($_SESSION["prihlaseni"]) || !isset($_SESSION["hrac"])) {
        ...
        <div style="text-align:center"><br><div style="height:75vh">
			<?php
				...
				if (isset($_GET["klic"])) {
					...
					if (isset($_SESSION["hrac"]) && $_SESSION["hrac"] == "s".$id) {
...
function submit() {
  ...
  xhttp.send("klic='.$klic.'&autor='.$_SESSION["idhrace"].'&zprava="+message);
	  }
...
function info() {
  ...
  xhttp.open("GET", "rinfo.php?klic='.$klic.'", true);
  xhttp.send();
}
...
function chat() {
  ...
  xhttp.open("GET", "rchat.php?klic='.$klic.'&hrac='.$_SESSION["idhrace"].'", true);
  xhttp.send();
}
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
	  ...
	  if (this.responseText.includes("peníze: , útok: , zdr")) {
		  window.location="sroom.php?klic='.$klic.'&quit=kick";
	  }
    }
  };
  xhttp.open("GET", "rhrac.php?klic='.$klic.'&hrac='.$_SESSION["idhrace"].'", true);
  xhttp.send();
}
...
function barva() {
  ...
  xhttp.open("GET", "rget.php?klic='.$klic.'&time", true);
  xhttp.send();
}
...
function title() {
  ...
  xhttp.open("GET", "rget.php?klic='.$klic.'&players", true);
  xhttp.send();
}
setInterval(title, 1000);
						...
						if (isset($_GET["quit"]) && !isset($_GET["odhlasit"])) {
							...
							unset($_SESSION['hrac']);
							if($_GET["quit"] == "kick") {echo "<br>Byl jsi kicknut.<style>#room{display:none;}</style>"; $_SESSION['forbidden'] = "s".$id;}
